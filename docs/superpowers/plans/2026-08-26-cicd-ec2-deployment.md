...
것은 아니다 — 대화형 SSH 세션에 이 블록을 통째로 붙여넣으면, 가드가 실패 메시지만 내고 넘어갈 뿐
셸은 계속 다음 줄을 받아들인다. 그 실패 메시지가 보였다면 이 블록의 어떤 명령도 실행되지 않았다는
뜻이니, Step 4의 `OLD=` 블록을 다시 실행해 값을 채운 뒤 이 블록부터 다시 붙여넣는다.
...
구 컨테이너가 실제로 없어졌는지 확인한 뒤에만 새 컨테이너를 올린다. 아래 확인이 "아직 남아 있다"로
나오면 그 다음의 `docker compose up` 줄을 실행하지 않는다 — 원인을 해결하고 이 확인을 다시 돌려
"제거 확인"이 나온 뒤에만 진행한다.
