...
3. **컨테이너 이름 `icuh_platform`은 추측이다.** 이전 프로젝트의 워크플로가 쓰던 이름을 그대로 적어
   ...
   그대로 재사용할 뿐 다시 묻지 않는다. `OLD`가 설정돼 있지 않으면(예: 접속이 끊겼다가 다시 붙어 셸
   세션이 바뀐 경우) Step 8의 가드(`: "${OLD:?...}"`)가 실패해 `&&`로 묶인 `docker stop`·`docker rm`이
   실행되지 않는다 — **셸 자체가 멈추는 것은 아니다.** 실패 메시지가 보이면 그 블록의 어떤 명령도
   실행되지 않은 것이니, Step 4의 `OLD=` 블록부터 다시 실행한다.
...
`OLD`가 이 셸 세션에 남아 있지 않으면(재접속 등으로 사라졌으면) 아래 첫 블록의 가드가 실패한다 —
`&&`로 묶여 있어서 그 뒤의 `docker stop`·`docker rm`은 실행되지 않는다. **가드가 셸 자체를 멈추는
것은 아니다** — 이 블록을 대화형 SSH 세션에 통째로 붙여넣으면, 가드가 실패 메시지만 내고 넘어갈
뿐 셸은 계속 다음 줄을 받아들인다. 화면에 그 실패 메시지가 보였다면 이 블록의 어떤 명령도 실행되지
않았다는 뜻이니, Step 4의 `OLD=` 블록을 다시 실행해 값을 채운 뒤 여기로 돌아와 이 블록부터 다시
붙여넣는다.
...
구 컨테이너가 실제로 없어졌는지 확인한 뒤에만 새 컨테이너를 올린다. **아래 확인이 "아직 남아
있다"로 나오면 그 다음의 `docker compose up` 줄을 실행하지 않는다** — 원인을 해결하고 이 확인을
다시 돌려 "제거 확인"이 나온 뒤에만 진행한다.

```bash
docker ps -a --filter "name=^/${OLD}$" --format '{{.Names}}' | grep -q . \
  && echo "아직 남아 있다. 위 명령의 출력을 확인하고 해결한 뒤 진행한다." \
  || echo "제거 확인 — 신규 컨테이너를 올린다"
...
- 브리프 원문의 Step 8은 `docker stop icuh_platform && docker rm icuh_platform`처럼 추측 컨테이너
  이름을 그 자리에 직접 적었다. 이름을 두 곳(백업 캡처와 이 명령)에 따로 적으면 운영자가 한쪽만
  고치고 다른 쪽을 놓칠 수 있어, Step 8도 Step 4에서 설정한 `$OLD`를 재사용하도록 바꿨다. 처음에는
  `: "${OLD:?...}"` 가드를 별도 줄로 두었는데, 그 형태는 비대화형 셸에서만 전체를 중단시키고 대화형
  SSH 세션에 블록을 붙여넣는 실제 상황에서는 가드가 실패 메시지만 내고 다음 줄(`docker stop ""`,
  `docker compose up -d`)로 그대로 넘어간다는 결함이 있어(빈 문자열로 `docker stop`이 우연히 실패해
  주는 것 말고는 아무것도 막지 못함), `&&`로 `docker stop`·`docker rm`을 가드에 체인으로 묶어 단락
  평가가 실제로 막도록 고쳤다. 재시작도 `docker ps -a --filter`로 구 컨테이너가 실제로 사라졌는지
  확인한 뒤에만 진행하도록 안내를 추가했다(운영자가 읽고 판단하는 수동 게이트다).
